...
`,document.body.appendChild(e),p.domAvatar=e.querySelector(".uuid-avatar"),p.domUserInfo=e.querySelector(".uuid-user-info-panel"),p.domSearchInput=e.querySelector(".uuid-search-input"),p.domSearchDropdown=e.querySelector(".uuid-search-dropdown"),p.domCardsContainer=e.querySelector(".uuid-cards-container"),p.domSettingsPanel=e.querySelector(".uuid-settings-panel"),p.domThemeToggle=e.querySelector(".uuid-theme-toggle"),p.domThemeToggle.classList.toggle("dark","dark"===p.currentTheme)},load:async()=>{p.domCardsContainer.innerHTML=`
            <div class="uuid-loading">
                <div class="uuid-loading-spinner"></div>
                <span>正在加载...</span>
            </div>
        `,await g.viewUser(),await g.viewLink();try{let e=await h.instanceUser.getItem(`${p.flagName}:update-time`);e&&Date.now()-e>6048e5&&(await g.reqUser(p.flagName,!0),await g.reqLibs(p.flagName,p.flagResp,p.flagLibs,!0))}catch(e){console.debug("Auto refresh cache failed:",e)}},bindEvent:()=>{document.body.addEventListener("click",async function(e){let t=e.target,r=t.closest("[data-action]");r&&g.triggerAction(r.dataset.action,r),t.closest(".uuid-user-section")||g.toggleDropdown(p.domUserInfo,!1),t.closest(".uuid-settings-dropdown")||g.toggleDropdown(p.domSettingsPanel,!1)}),p.domSearchInput.addEventListener("keydown",function(e){if(["ArrowUp","ArrowDown"].includes(e.code))e.preventDefault(),g.searchArrow(e.code);else if("Enter"===e.key){e.preventDefault();let t=p.domSearchDropdown.querySelector(".active");t&&t.click()}}),p.domSearchInput.addEventListener("input",function(){g.searchLink(this.value.trim())}),p.domSearchInput.addEventListener("focus",function(){this.classList.add("focused")}),p.domSearchInput.addEventListener("blur",function(){this.classList.remove("focused")}),document.body.addEventListener("keydown",function(e){if("Escape"===e.code){e.preventDefault(),p.domSearchInput.value="",g.updateSilentSearch(""),g.toggleDropdown(p.domSearchDropdown,!1),p.domSearchInput.blur();return}if(document.activeElement&&!["INPUT","TEXTAREA"].includes(document.activeElement.tagName))if(e.ctrlKey&&["KeyQ","KeyK"].includes(e.code))e.preventDefault(),p.domSearchInput.focus(),g.updateSilentSearch("");else if("Backspace"===e.code)e.preventDefault(),g.updateSilentSearch(p.domSearchInput.dataset.search.slice(0,-1));else if("Enter"===e.key){if(e.preventDefault(),p.domSearchDropdown.classList.contains("show")){let e=p.domSearchDropdown.querySelector(".active");e&&e.click()}}else["ArrowUp","ArrowDown"].includes(e.code)?(e.preventDefault(),p.domSearchInput.dataset.search.length&&g.searchArrow(e.code)):/^[a-z0-9\.\_\-\/]$/i.test(e.key)&&!e.ctrlKey&&(p.domSearchInput.value="",document.documentElement.scrollTo(0,0),g.updateSilentSearch(p.domSearchInput.dataset.search+e.key))})},triggerAction:async(e,t)=>{switch(e){case"toggle-user-info":g.toggleDropdown(p.domUserInfo);break;case"toggle-settings":g.toggleDropdown(p.domSettingsPanel);break;case"theme":g.toggleTheme(),p.domThemeToggle.classList.toggle("dark","dark"===p.currentTheme);break;case"token":{let e=`
                        <div class="uuid-form-group">
                            <label class="uuid-label">GitHub Token</label>
                            <input class="uuid-input" type="password" placeholder="ghp_xxxxxxxxxxxxxxxxxxxx" value="${d.htmlOf(p.flagToken||"")}" />
                    ...
                    `,t=g.createModal("设置 GitHub Token",e,[{text:"取消",action:"close-modal"},{text:"保存",action:"save-token",class:"uuid-btn-primary"}]);t.addEventListener("click",async e=>{if("save-token"===e.target.dataset.action){let e=t.querySelector("input").value.trim();if(""===e)p.flagToken=null,await h.removeItem("uuid-token-github"),g.showNotification("Token已清除","success");else{if(!(e.length>10))return void g.showNotification("Token格式不正确","error");p.flagToken=e,await h.setItem("uuid-token-github",e),g.showNotification("Token已保存","success")}g.closeModal(t);let r=document.querySelector(".uuid-token-indicator");r&&r.classList.toggle("active",null!=p.flagToken)}})}break;case"local":{let e=`
                        <p>确定要切换到 ${p.flagLocalUsed?"GitHub 线上":"本地"} 模式吗？</p>
                        <div style="margin: 16px 0; padding: 12px; background: var(--bg-secondary); border-radius: 6px; font-size: 14px;">
                            <strong>当前：</strong> ${p.flagLocalUsed?"本地模式":"GitHub 线上模式"}<br>
                            <strong>切换到：</strong> ${p.flagLocalUsed?"GitHub 线上模式":"本地模式"}
                        </div>
                    `;g.createModal("切换模式",e,[{text:"取消",action:"close-modal"},{text:"确定",action:"confirm-local",class:"uuid-btn-primary"}]).addEventListener("click",async e=>{"confirm-local"===e.target.dataset.action&&(p.flagLocalUsed=!p.flagLocalUsed,await h.setItem("local",p.flagLocalUsed),"#_local"===location.hash&&(location.hash=""),location.reload())})}break;case"proxy":p.flagProxyUsed=!p.flagProxyUsed,await h.setItem("proxy",p.flagProxyUsed),t.querySelector(".uuid-toggle-switch").classList.toggle("active",p.flagProxyUsed),g.showNotification(p.flagProxyUsed?"GitHub 代理已启用":"GitHub 代理已禁用","success");break;case"reload":for(let e of(await h.instanceUser.keys()))e.startsWith(`${p.flagName}:`)&&await h.instanceUser.removeItem(e);await g.load();break;case"convert":{let e=`
                        <div class="uuid-form-group">
...
`),g.showNotification("转换完成","success")}catch(e){console.error(e),g.showNotification("转换失败","error")}})}break;case"about":{let e=`
                    ...
                    `;g.createModal("关于",e,[{text:"关闭",action:"close-modal"}])}}},reqUser:async(e,t)=>{let r=`https://api.github.com/users/${e}`,a=`${e}:${r}`,n=await h.instanceUser.getItem(a);return(null==n||t)&&(n=await g.reqServer(r))&&(await h.instanceUser.setItem(a,n),await h.instanceUser.setItem(`${e}:update-time`,Date.now())),n},reqLibs:async(e,t,r,a)=>{let n=`https://api.github.com/repos/${e}/${t}/contents/${r}`,o=`${e}:${n}`,i=await h.instanceUser.getItem(o);return(null==i||a)&&(i=await g.reqServer(n))&&await h.instanceUser.setItem(o,i),i},reqRaw:async e=>{let t=`${p.flagName}:${e}`,r=await h.instanceUser.getItem(t);return null==r&&(r=await g.reqServer(`${e}?_${d.random()}`,{type:"text"}))&&await h.instanceUser.setItem(t,r),r},convertHtml:(e,t)=>{for(let r=0;r<e.children.length;r++){let a=e.children[r];switch(a.nodeName){case"H3":t.push(""),t.push("### "+a.innerHTML);break;case"DL":case"P":g.convertHtml(a,t);break;case"DT":if(1===a.children.length){let e=a.querySelector("a");if(e){let r="["+e.innerHTML.replace(/`/g,"\\`")+"]("+e.href;e.title&&(r+=' "'+e.title+'"'),t.push("- "+r+")")}}else g.convertHtml(a,t)}}},viewUser:async()=>{let e;if(p.flagLocalUsed?(p.flagLocalJson=await g.reqServer(p.flagLocalPath),p.flagLocalJson?e=p.flagLocalJson.user:g.showNotification("加载失败","error")):e=await g.reqUser(p.flagName),e){p.domAvatar.onerror=function(){p.domAvatar.src="/favicon.ico",p.domAvatar.onerror=null},p.domAvatar.src=e.avatar_url;let t=d.htmlOf(e.name||e.login),r=e.bio?d.htmlOf(e.bio):"",a=e.company?d.htmlOf(e.company):"",n=e.location?d.htmlOf(e.location):"",o=e.blog?d.htmlOf(e.blog):"",i=d.htmlOf(e.login),s=`
            ...
            `;p.domUserInfo.innerHTML=s,document.title=`${e.login} - ${p.flagTitle}`}},emptyState:(e,t,r)=>{let a={empty:'<circle cx="12" cy="12" r="10"></circle><path d="M12 6v6l4 2"></path>',error:'<circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line>'};return`
            <div class="uuid-empty">
                <svg class="uuid-empty-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ${a[e]||a.error}
                </svg>
                <div class="uuid-empty-title">${d.htmlOf(t)}</div>
                <div class="uuid-empty-description">${d.htmlOf(r)}</div>
            </div>
        `},parseSections:e=>{let t=document.createElement("div");t.innerHTML=e;let r=[],a=null;return t.childNodes.forEach(e=>{e.nodeType===Node.ELEMENT_NODE&&("H3"===e.tagName?(a&&r.push(a),a={title:e.textContent,links:[]}):("UL"===e.tagName||"OL"===e.tagName)&&(a||(a={title:null,links:[]}),e.querySelectorAll("a").forEach(e=>{let t=e.textContent.trim();if(t.includes("http")){let e=t.split(/\s+/);e.length>1&&/^https?:\/\//.test(e[e.length-1])&&(t=e.slice(0,-1).join(" "))}a.links.push({title:t,url:e.href,tip:e.title||""})})))}),a&&r.push(a),r},renderSections:e=>e.map(e=>`
            <div class="uuid-card-section">
                ...
                <div class="uuid-links-grid">
                    ${e.links.map(e=>`
                        <a href="${d.htmlOf(e.url)}" target="_blank" class="uuid-link"${e.tip?` title="${d.htmlOf(e.tip)}"`:""}>
                            ${d.htmlOf(e.title)}
                        </a>
                    `).join("")}
                </div>
            </div>
        `).join(""),viewLink:async()=>{let e;if(p.flagLocalUsed?p.flagLocalJson&&(e=p.flagLocalJson.libs):e=await g.reqLibs(p.flagName,p.flagResp,p.flagLibs),e){let t=e.filter(e=>"file"===e.type&&e.name.endsWith(p.flagSuffix)),r=t.map((e,t)=>{let r=e.name.substring(0,e.name.length-p.flagSuffix.length);return`
                    <div class="uuid-card" id="card-${t}">
                        <div class="uuid-card-header">
                            <h3 class="uuid-card-title">${d.htmlOf(r)}</h3>
                        </div>
                ...
                `});p.domCardsContainer.innerHTML=r.join("");let a=await s.e(394).then(s.bind(s,394));for(let e=0;e<t.length;e++){let r=t[e],n=document.querySelector(`#card-${e} .uuid-card-content`);try{let t=await g.reqRaw(r.download_url);if(t){let r=g.parseSections(a.parse(t));n.innerHTML=g.renderSections(r)||g.emptyState("empty","暂无内容","此文件暂无有效的链接内容");let o=r.reduce((e,t)=>e+t.links.length,0),i=document.querySelector(`#card-${e} .uuid-card-title`);i&&o>0&&(i.innerHTML+=` <span class="uuid-card-count">(${o})</span>`)}else n.innerHTML=g.emptyState("error","加载失败","无法加载此文件的内容")}catch(e){console.error("Error loading card content:",e),n.innerHTML=g.emptyState("error","加载错误","处理文件内容时出现错误")}}p.domCardsContainer.dataset.ended=!0}else p.domCardsContainer.innerHTML=g.emptyState("error","加载失败","无法获取收藏夹列表")},updateSilentSearch:e=>{p.domSearchInput.dataset.search=e,p.domSearchInput.placeholder=e.length?e:"静默搜索 (Ctrl+K)...",g.searchLink(e)},searchArrow:e=>{let t=p.domSearchDropdown.querySelectorAll(".uuid-search-item");if(0===t.length)return;let r=Array.from(t),a=r.findIndex(e=>e.classList.contains("active"));a>=0&&r[a].classList.remove("active"),"ArrowUp"===e?a=a<=0?t.length-1:a-1:"ArrowDown"===e&&(a=a>=t.length-1?0:a+1),a>=0&&a<t.length&&r[a].classList.add("active")},searchLink:e=>{if(p.domCardsContainer.dataset.ended){let t=[];if(""!==e.trim()){let r=e.toLowerCase(),a=p.domCardsContainer.querySelectorAll("a");for(let e=0;e<a.length;e++){let n=a[e];if(`${n.href},${n.textContent},${n.title||""}`.toLowerCase().includes(r)&&(t.push({url:n.href,title:n.textContent.trim(),tip:n.title||""}),t.length>=7))break}}if(t.length>0){let e=t.map((e,t)=>`
                    <a href="${d.htmlOf(e.url)}" target="_blank" class="uuid-search-item ${0===t?"active":""}"${e.tip?` title="${d.htmlOf(e.tip)}"`:""}>
                        <div class="uuid-search-item-url">${d.htmlOf(e.url)}</div>
                        <div class="uuid-search-item-title">${d.htmlOf(e.title)}</div>
                    </a>
                `).join("");p.domSearchDropdown.innerHTML=e,g.toggleDropdown(p.domSearchDropdown,!0)}else g.toggleDropdown(p.domSearchDropdown,!1)}},reqServer:async(e,t)=>{t=t||{},null!=p.flagToken&&p.flagToken.length>10&&!e.includes("githubusercontent")&&(t.headers=t.headers||{},t.headers.authorization=`token ${p.flagToken}`),p.flagProxyUsed&&!p.flagLocalUsed&&(e=`${p.flagProxyServer}${encodeURIComponent(e)}`);let r=await d.fetch(e,t);if(r.error)console.error("Network error:",r.error),g.showNotification("网络错误","error");else if(r.resp.ok){if(r.result)return r.result}else 403===r.resp.status?g.showNotification("请设置 Token","warning"):g.showNotification(`请求失败 (${r.resp.status})`,"error")}};Object.assign(window,{nrWeb:g}),"loading"==document.readyState?document.addEventListener("DOMContentLoaded",g.init):g.init()})();